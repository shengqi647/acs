...
from adkg.utils.serilization1 import serialize_g, deserialize_g, serialize_gs, deserialize_gs, deserialize_f
import hashlib
...
def test_benchmark_reconstruction(benchmark, t, n):
    ...
    reconstruct( phis, n, t, commits[0], g, field)
...
def reconstruct(sharess, n, t, commits, g1, CoinZR):
    ...
    for k, phi_k in shares.items():
        ...
        # commits2 = commits[:len(commits)-1]
        # powers = [CoinZR((k+1)**j) for j in range(len(commits))]
        # lhs = multiexp(commits, powers)
