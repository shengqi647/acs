    ...
    if not only_setup:
        ...
        instance_commands = [
            [
                instance_id,
                [
                    # f"rm /home/ubuntu/benchmark-logs/benchmark.log && sudo docker stop $(sudo docker ps -aq) && sudo docker rm $(sudo docker ps -aq) && "
                    f"sudo docker run\
                -p {port}:{port} \
                ...
                {AwsConfig.MPC_CONFIG.COMMAND} -d -f config/config-{i}.json -v -time {start_time}"
                ],
            ]
            for i, instance_id in enumerate(instance_ids)
        ]
