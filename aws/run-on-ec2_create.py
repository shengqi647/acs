...
def trigger_run(run_id, skip_setup, max_k, only_setup, cleanup):
    ...
    with open("config0", "w+") as file:
        for instance_id in instance_ids:
            region_config = AwsConfig.REGION[ec2manager.instanceIdRegion[instance_id]]
            ip = ec2manager.get_instance_public_ip(instance_id)
            file.write(region_config.KEY_FILE_PATH+' '+ip+'\n')

    with open("config1", "w+") as file:
        for instance_id in instance_ids:
            region_config = AwsConfig.REGION[ec2manager.instanceIdRegion[instance_id]]
            ip = ec2manager.get_instance_private_ip(instance_id)
            file.write(region_config.KEY_FILE_PATH+' '+ip+'\n')
    #
    # if cleanup:
    #     instance_commands = [
