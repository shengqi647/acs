...
async def optqrbc_u(sid, pid, n, f, leader, predicate, input, output, send, receive, echo2_diable, view):
    """
    ...
    echo2_counter = defaultdict(lambda: 0)
    ...
    while True:  # main receive loop
        try:
            ...
            if msg[0] == RBCMsgType.PROPOSE and leader_hash is None:
                ...
                if view > 0:
                    import pickle
                    __, _, ___= pickle.loads(leader_msg)
                    leader_msg = pickle.dumps(__)
                ...
                if echo_counter[_digest] >= echo_threshold and not echo2_sent:
                    echo2_sent = True
                    broadcast((RBCMsgType.ECHO2, _digest))

            elif msg[0] == RBCMsgType.ECHO2 and echo2_diable[0] == False:
                (_, _digest) = msg
                if sender in echo2_senders:
                    # Received redundant ECHO message from the same sender
                    continue
                echo2_senders.add(sender)
                echo2_counter[_digest] = echo2_counter[_digest] + 1

                if echo2_counter[_digest] >= echo2_threshold and not ready_sent:
                    ready_sent = True
