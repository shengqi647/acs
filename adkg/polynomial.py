...
def polynomials_over(field):
    ...
    class Polynomial(object):
        ...
        def interpolate_fft(cls, ys, omega):
            ...
            coeffs = [b / n for b in fft_helper(ys, field(1) / omega, field)]
            return cls(coeffs)
        ...
        @classmethod
        def random_seed(cls, degree, y0=None, seed=None):
            coeffs = [field.random(seed=seed) for _ in range(degree + 1)]
            if y0 is not None:
                if type(y0) is int:
                    y0 = field(y0)
                assert type(y0) is field_type
