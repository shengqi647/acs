...
pub mod bls12_381;
use bls12_381::{G1, G1Affine, G1Compressed, G1Uncompressed, G2Compressed, G2, Fr, Fq, Fq2, Fq6, Fq12, FqRepr, FrRepr};
...
impl PyG1 {
    ...
    pub fn gets<'p>(&self, py: Python<'p>) -> PyResult<&'p PyBytes> {
        let aff = self.g1.into_affine();
        let compressed = aff.into_uncompressed();
        Ok(PyBytes::new(py, compressed.as_ref()))
        /*let fqx = FqRepr::from(aff.x);
        let fqy = FqRepr::from(aff.y);
        let arr1: &[u64] = fqx.as_ref();
    ...
    pub fn sets(&mut self, list: &PyAny) -> PyResult<()>
    {
        ...
        let start_time = Instant::now();
        let compressed = G1Uncompressed(bytes);
        println!("Time {:?}", start_time.elapsed());
        let start_time = Instant::now();
